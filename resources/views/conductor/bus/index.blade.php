...
<div class="col-md-6 text-right">
    <a class="btn btn-success" href="{{ route('cbus.create')}}" >{{  trans('sentence.add bus')}}</a>
</div>
<table class="table table-responsive  table-hover table-bordered">
  ...
  <tbody>
    ...
    <tr class="table-success">
      ...
      <td>
                <a href="{{ route('cbus.edit',$bus->id)}}" class="btn btn-primary">{{  trans('sentence.edit')}}</a>
            </td>
            <td>
                <form action="{{ route('cbus.destroy', $bus->id)}}" method="post">
                  @csrf
                  @method('DELETE')
