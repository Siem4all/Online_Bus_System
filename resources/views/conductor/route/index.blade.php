...
<div class="row">
    <div class="col-md-6">
<form action="/crsearch" method="GET" role="form">
...
<div class="col-md-6 text-right">
    <a class="btn btn-success" href="{{ route('croute.create')}}" >{{  trans('sentence.add route')}}</a>
</div>
  ...
  <tbody>
    ...
    <tr class="table-success">
      ...
      <td>{{$route->Departure}}</td>
        ...
        <td>{{$route->Fare}}</td>
      <td>
                <a href="{{ route('croute.edit',$route->id)}}" class="btn btn-primary">{{  trans('sentence.edit')}}</a>
            </td>
            <td>
                <form action="{{ route('croute.destroy', $route->id)}}" method="post">
                  @csrf
                  @method('DELETE')
